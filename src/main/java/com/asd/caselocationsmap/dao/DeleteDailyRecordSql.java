...
public class DeleteDailyRecordSql {
    ...
    public void removeDailyRecordByEmail(DailyRecord dr) {
        try{
            String username = "asd";
            String password = "ASDpassword";
            String connectionUrl = "jdbc:mysql://aa1k7pic2tpxd6q.cn9vqfnivxwa.us-east-2.rds.amazonaws.com:3306/CovidMap";
            Connection conn = DriverManager.getConnection(connectionUrl, username, password);
            String sql = "delete from DailyCOVIDRecord  where publish_date='"+dr.getPublishDate()+"';";
            System.out.println("6666666666666dasdadadasdas");
            System.out.println(dr.getPublishDate());
            Statement stmt = conn.createStatement();
            stmt.executeUpdate(sql);
            conn.close();
        } catch (SQLException e) {
